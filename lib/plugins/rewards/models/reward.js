...
exports.plugin = function(mongodb) {
	...
	RewardSchema.statics.addOne = function(options, next) {
		...
		points = rewardTypeInfo.points || options.points || 0;

		if(!points) return next(new Error("it's not an award if the user doesn't get anything, is it?"));
		...
		step(
			...
			on.success(function(reward) {
				...
				this.reward.status = options.status || (this.reward.completed ? "Completed" : this.reward.status);
