...
exports.plugin = function(mongodb, auth, stripe, plans, loader) {
	...
	CustomerSchema.statics.setCreditCard = function(options, callback) {
		...
		step(
			...
			function() {
				...
				Customer.getCustomer(options, this);
			},
	...
	CustomerSchema.statics.getCustomer = function(options, callback) {
		var on = outcome.error(callback),
		Customer = this;
		...
		step(
			...
			function() {
				...
				if(options.owner) {
					Account.findOne({ _id: options.owner }, this);
				}
			},
			...
			on.success(function(customer) {
				...
				if(this.customer = customer) {
					...
					options.email = this.account.email
					console.log("creating stripe customer");
					stripe.customers.create(_.pick(options, "email", "card", "coupon", "description", "plan", "trial_end", "quantity"), this);
					delete options.card; //delete this incase 
					delete options.plan;
				}
	...
	CustomerSchema.methods.getAccount = function(next) {
		Account.findOne({ _id: this.owner }, next);
	}
